...
Richard TRÉZEUX
adult_to_json.py
Preprocess the Adult dataset and create .json file that are used in the FLASH platform.
...
def main():
    ...
    if gan:
        print("GAN/Variational Autoencoder setup...")
        if niid:
            print("100/0 Non-IID setup... with sensitive attribute : ", sensattr)
            users, num_samples, user_data = to_leaf_format_gan_100_0(df_adult, labels, num_workers, sensattr, size = gan)
            ...
            users, num_samples, user_data = to_leaf_format_gan(df_adult, labels, num_workers, size = gan)
...
def get_indices_workers_gan_100_0(df, num_workers, sensattr, size = 3): # N_samples race attribute ['0' : 6796, '1' : 41189] // N_samples gender/sex attribute ['0' : 15944, '1' : 32041]
    '''We take 'size' samples of each value of sensitive attributes (2 binary sensitive attributes) in order to have diversity in the little subset of data.
    Therefore the size of the subset of data that will be shared is 4 * size '''
    if sensattr == "race": 
        n0, n1 = 6796, 41189
