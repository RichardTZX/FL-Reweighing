...
Richard TRÉZEUX
adult_to_json.py
Preprocess the COMPAS dataset and create .json file that are used in the FLASH platform.
More details on https://github.com/RichardTZX/FL-Reweighing
"""
import json
import numpy as np
import os
...
def main():
    ...
    gansize, niid, sensattr  = args.gan, args.niid, args.sens_attr

    dir_path = os.path.join(parent_path, 'data', 'all_data')

    if gansize:
        if niid:
            users, num_samples, user_data = to_leaf_format_gan_100_0(df_compas, labels, num_workers, sensattr, size = gansize)
            ...
            users, num_samples, user_data = to_leaf_format_gan(df_compas, labels, num_workers, size = gansize)
...
def get_indices_workers_gan(df, num_workers, size = 3):
    '''We take 'size' samples of each value of sensitive attributes (2 binary sensitive attributes) in order to have diversity in the little subset of data.
    Therefore the size of the subset of data that will be shared is 4 * size '''
    ...
    gan_ind = gan_indices(df, indices, size = size)
    delete_indices = []
...
def get_indices_workers_100_0(df, num_workers, sensattr): # N_samples race attribute ['0' : 3175, '1' : 2103] // N_samples gender/sex attribute ['0' : 4247, '1' : 1031]
    if sensattr == "race": 
        n0, n1 = 3175, 2103
    else: #sensattr == "sex"
        n0, n1 = 4247, 1031
...
def get_indices_workers_gan_100_0(df, num_workers, sensattr, size = 3): # N_samples race attribute ['0' : 3175, '1' : 2103] // N_samples gender/sex attribute ['0' : 4247, '1' : 1031]
    '''We take 'size' samples of each value of sensitive attributes (2 binary sensitive attributes) in order to have diversity in the little subset of data.
    Therefore the size of the subset of data that will be shared is 4 * size '''
    if sensattr == "race": 
        n0, n1 = 3175, 2103
...
def to_leaf_format_gan(df, labels, num_workers, size = 3):
    users, num_samples, user_data = [], [], {}
    indices_workers = get_indices_workers_gan(df,num_workers, size = size)
...
def to_leaf_format_gan_100_0(df, labels, num_workers, sensattr = "race", size = 3):
    users, num_samples, user_data = [], [], {}
    indices_workers = get_indices_workers_gan_100_0(df,num_workers, sensattr, size = size)
...
def parse_args():
	...
	parser.add_argument(
		'-gan',
		help='GAN setup (add a subset of data of size (4 * this parameter) to all workers);',
		type=int,
		default=None,
		required=False)
	parser.add_argument(
